...
def set_language_from_url(request, language_code):
    ...
    try:
        ...
        try:
            ...
            prev_page = Page.objects.get(url_path=prev_url_path)

            # if the current page has a canonical page, use that, otherwise the current page itself is canonical
            can_page = prev_page

            # if the requested language is the canonical (default) language, use the canonical page,
            # else find the translated page
            language = Locale.objects.get(code=language_code)
            next_url = can_page.url if language_code == settings.LANGUAGE_CODE \
                else Page.objects.get(locale=language).is_default_site.url
        except (Page.DoesNotExist, Locale.DoesNotExist):
            # previous page is not a TranslatablePage,
            # try if previous path can be translated by changing the language code
